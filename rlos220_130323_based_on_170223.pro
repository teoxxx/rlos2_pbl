...
DATAPATH='E:\lowres_55_deg_losu_sl_motion_theta0\torblob18dummies\'
cd, datapath
...
;
;
;130323 made selective udopplerfactor_4d
;
;170223 corrected freq shift in sync emiss and abs global calc and global absorption routines
;
;nobs=nobs_external/dopplerfactor_4d
...
if(synchrotron_emissivity_switch eq 1.0) then begin
;170223 added freqshift in sync abs
if (freqshiftchoice_external eq 1.0) then begin
nobs=nobs_external/dopplerfactor_4d
endif

(  kabs_4d=fast_proton_profile_4d*kappa_value_power_law*(c6_pachz(gammac))*rho_4d*( ( bfield_4d*sinlosb_4d )^( (gammac+2.)/2.) )*((nobs/2.)/c1_pachz)^((-4.-gammac)/2.)   )
;stop
endif
;;kabs=cp6(0.)*ndens*(bfield*sin(theta))^((gammac+2.)/2.)*((ng/2.)/cp1(0.))^(-(gammac+4.)/2.)
...
pro global_calculations,phi1_fun,phi2_fun,vx1_4d,vx2_4d,vx3_4d,v_4d,speedtweakfactor,coslosu_4d,lx1,lx2,lx3,thetau_4d,gammalorentz_4d,ccccc,ncalc,ng,dopplerfactor_4d,debug_comments_fun,focused_beam_switch_fun, bx1_4d, bx2_4d, bx3_4d, coslosb_4d, sinlosb_4d, bfield_4d, dopplerchoice_external
...
;130323 can opt out of dfactor using param file, mainly for testing purposes
if (dopplerchoice_external eq 1.0) then (dopplerfactor_4d=1/( gammalorentz_4d*(1-ccccc*coslosu_4d) )) else dopplerfactor_4d=(dopplerfactor_4d+1.0)/(dopplerfactor_4d+1.0)
...
;stop
(  emiss_4d=(kappa_value_power_law*c5_pachz(gammac))*rho_4d*( ( bfield_4d*sinlosb_4d )^( (gammac+1.)/2.) )*((nobs/2.)/c1_pachz)^((1.-gammac)/2.)*(dopplerfactor_4d*dopplerfactor_4d)   )
endif
;170722 testing proved dfactor squared has many nans, so drop it for now then fix it
;180722 sos no problem, we just had the speed multiplier too high, 2.88. set it back to unity, and guess what, it now works like a charm!
...
;stop
(  emiss_4d=fast_proton_profile_4d*(kappa_value_power_law*c5_pachz(gammac))*rho_4d*( ( bfield_4d*sinlosb_4d )^( (gammac+1.)/2.) )*((nobs/2.)/c1_pachz)^((1.-gammac)/2.)*(dopplerfactor_4d*dopplerfactor_4d))
endif
...
global_calculations,phi1,phi2,vx1_4d,vx2_4d,vx3_4d,v_4d,speedtweakfactor,coslosu_4d,lx1,lx2,lx3,thetau_4d,gammalorentz_4d,ccccc,ncalc,ng,dopplerfactor_4d,debug_comments,focused_beam_switch, bx1_4d, bx2_4d, bx3_4d, coslosb_4d, sinlosb_4d, bfield_4d, dopplerchoice_external
...
stop
surface, eikona, ax=75, az=180,xcharsize=3, ycharsize=3, zcharsize=0
stop
